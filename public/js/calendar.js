(function($) {
    ...
    $(document).ready(function() {
        ...
        $(".fc-button-agendaDay").on("click", function() {
            ...
            var vis = d3.select('#graph').append("svg").data([data]).attr("width", Gw).attr("height", Gh).append("g").attr("transform", "translate(" + Gr + "," + Gr + ")"); // 원의 중심 지정
            var arc = d3.svg.arc().outerRadius(Gr).innerRadius(Gr - 130);
            var pie = d3.layout.pie().value(function(d) {
                return d.value;
            }).sort(null);
            var paths = vis.selectAll(".slice").data(pie).enter().append("g").attr("class", "slice").append("path").each(function(d, i) { // 각 파이의 인덱스 지정
                d3.select(this).attr("index", i)
            }).on("mousedown", mousedown).on("mouseover", mouseover).on("mouseup", mouseup);
            var tooltip = d3.selectAll(".slice").append('div').attr('class', 'tooltip');
            tooltip.append('div').attr('class', 'label');
            tooltip.append('div').attr('class', 'percent');
            paths.transition().duration(500).attr("d", arc).each(function(d) {
                this._current = d;
            });
            var dragging;
            var sumValue = 0;
            ...
            var svg = d3.select("#graph").append("svg").attr("width", Cw + 30).attr("height", Ch + 30).attr("x", 60).attr("y", 60).style("fill", "white").style("opacity", "0.8");
            var face = svg.append('g').attr('id', 'clock-face').attr('transform', 'translate(' + (clockRadius + margin) + ',' + (clockRadius + margin) + ')');
            face.selectAll('.hour-label').data(d3.range(3, 13, 3)).enter().append('text').attr('class', 'hour-label').attr('text-anchor', 'middle').attr('x', function(d) {
            ...
            face.append('g').attr('id', 'face-overlay').append('circle').attr('class', 'hands-cover').attr('x', 0).attr('y', 0).attr('r', clockRadius / 60);
            ...
            var startIndex = 0;
            var time;
            var target; // 데이터로 만들 때 스타트 path 인덱스
            // 데이터로 path 그리기
                ...
                for (var i = 0; i < eventsArray.length; i++) {
                    ...
                    if (setD[0] == date) { // same Date
                        if (endD[0] == date) {
                            ...
                            count = Number(count);
                            // 30씩 더해줌
                            // mousedown, mouserover, mouseup의 기능을 모두 추가
                                ...
                                if (NsetT == data[j].label) { // same start time
                                    for (var k = j - 1; k < 24; k++) { // k=endTime
                                        if (NendT == data[k].label) { // same end time
                                            ...
                                            }).addClass("biggerFilling").attr("sumIndex", sumIndex)
                                              .attr("makeEdit", "true").attr("index"); // startPath 지정
            ...
            function change(target) {
                    ...
                    paths.transition().duration(750).attrTween("d", arcTween); // path 다시 그림
                }
            ...
            function editModal(d) {
                ...
                $("#remove_day").off('click').on('click', function hey() {
                    ...
                    if (targetPath.hasClass("smallerFilling")) {
                        ...
                        if (targetPath.attr("makeEdit") == "true")
                            for (var i = seperIndex[1]-1; i < lastnum; i++) calcPath(i);
                        else
                            for (var i = seperIndex[1]; i < lastnum + 2; i++) calcPath(i);
                    }
            ...
            d3.select(self.frameElement).style("height", Ch + "px");
                ...
                function calcPath(i) {
                    if (i % 2 == 0) // 2의 배수이면
                        data[i].label = i / 2 + ":00";
                    else data[i].label = Math.floor(i / 2) + ":30";
                    data[i].value = 30;
                    paths.data(pie(data));
